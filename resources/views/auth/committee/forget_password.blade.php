            ...
            <div class="col-12">
                <div class="codex-authbox mt-4 mb-4">
				   ...
				   <p class="alert alert-danger">{{ Session::get('msg') }}</p>
                    @endif --}}
                    <form method="POST" action="{{ route('comm.commSubmitForgetPassword') }}">
                        @csrf
                        <div class="form-group">
                            <label for="emailaddress" class="form-label">{{ __('Email address') }}<span class="text-danger">*</span></label>
                            <input type="email" name="email" class="form-control" id="emailaddress" autocomplete="off" placeholder="Enter your email" />
                            @if($errors->has('email'))
...
<script type="text/javascript">
    $('#reload').click(function () {
        $.ajax({
            type: 'GET',
            url: '{{route('comm.reloadCaptchaCommForget')}}',
