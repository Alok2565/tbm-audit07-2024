            ...
            <div class="col-12">
                <div class="codex-authbox mt-4 mb-4">
                    <div class="auth-header">
                        ...
                        <h5 class="justify-content-between" style="color: #000; font-weight:600;">Login for
                            Committee Members</h5>
                    </div>
                    @if(Session::has('msg'))
                    <p class="alert alert-danger">{{ Session::get('msg') }}</p>
                    @endif
                    <form method="POST" action="{{ route('committee.validate_login') }}">
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            <span class="float-end"><a href="{{route('comm.commShowForgetPwdForm')}}">{{ __('Reset Password') }}</a></span>
                            <input id="password" type="password" name="password"
                                class="form-control @error('password') is-invalid @enderror" placeholder="Password"
                                autocomplete="off" />
                           <!--<input id="pass_enc" value="" type="hidden" name="pass_enc" value="iphshealth"
                        ...
                        <div class="form-group">
                            <button class="btn btn-primary" type="submit" onclick="encryptPassword();"><i
                                    class="fa fa-sign-in"></i> {{ __('Login') }}</button>
                        </div>
...
$('#reload').click(function() {
    $.ajax({
        type: 'GET',
        url: '{{route('login.reloadCaptchaCommittee')}}',
