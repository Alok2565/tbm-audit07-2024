          ...
          <div class="col-12">
            <div class="codex-authbox mt-4 mb-4">
              <div class="auth-header">
                ...
                <h5 class="justify-content-between" style="color: #000; font-weight:600;">Login for ICMR Officers</h5>
              </div>
              @if(Session::has('msg'))
				<p class="alert alert-danger">{{ Session::get('msg') }}</p>
			 @endif
              <form method="POST" action="{{ route('icmr.validate_login') }}">
                @csrf
                <div class="form-group">
                    <label class="form-label" for="iec_code">{{ __('Email Id') }}</label>
                    <input type="email" name="email" class="form-control" placeholder="Email Id"  autocomplete="off" required/>
                    @if($errors->has('email'))
                ...
                <div class="form-group">
					...
					<span class="float-end"><a href="{{url('icmr/forget-password')}}">Reset Password</a></span>
 ...
 const passwordInput = this.querySelector("input[type=password]");
    ...
    <script type="text/javascript">
        $('#reload').click(function () {
            $.ajax({
                type: 'GET',
                url: '{{route('icmr_login.reloadCaptcha')}}',
