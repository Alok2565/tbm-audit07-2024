            ...
            <div class="col-12">
                <div class="codex-authbox mt-4 mb-4">
				   ...
				   <p class="alert alert-danger">{{ Session::get('msg') }}</p>
                    @endif --}}
                    <form method="POST" action="{{ route('nic.submitResetPasswordNic') }}">
                        @csrf
                          {{-- <input type="hidden" name="token" value="{{ $token }}"> --}}

                          <div class="form-group">
                            <label for="emailaddress" class="form-label">{{ __('Email address') }}<span class="text-danger">*</span></label>
                            <input type="text" id="email_address" class="form-control" autocomplete="off" name="email" value="{{ $email }}" required autofocus placeholder="Email" readonly>
                                  @if ($errors->has('email'))
                          ...
                          <div class="form-group">
                              ...
                              <input type="password" id="password" autocomplete="off" class="form-control" name="password" required autofocus placeholder="Password">
                          ...
                          <div class="form-group">
                              ...
                              <input type="password" id="password-confirm" autocomplete="off" class="form-control" name="password_confirmation" required autofocus placeholder="Confirm Password">
...
<script type="text/javascript">
    $('#reload').click(function () {
        $.ajax({
            type: 'GET',
            url: '{{route('icmr.reloadCaptchaIemrReset')}}',
